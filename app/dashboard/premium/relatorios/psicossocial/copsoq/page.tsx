...
type ClienteOption = { id: string; nome: string | null };
...
export default function CopsoqDashboardPage() {
  ...
  // Admin: lista de clientes e seleção
  ...
  // - admin: selectedClienteId
  const effectiveClienteId = useMemo(() => {
    if (isCliente) return me?.cliente_id ?? null;
    return selectedClienteId ?? null;
  }, [isCliente, me?.cliente_id, selectedClienteId]);

  // 1) carrega me + lista de clientes (admin)
  useEffect(() => {
    let mounted = true;
    ...
    (async () => {
      ...
      try {
        ...
        // Bloqueio (gestor/usuario) — igual RN
        if (authUser.role === "gestor" || authUser.role === "usuario") {
          setRows([]);
          return;
        }
        ...
        if (authUser.role === "cliente") {
          setSelectedClienteId(authUser.cliente_id ?? null);
        }

        // Admin: carrega lista de clientes (para filtro)
        if (authUser.role === "admin") {
          const { data: lista, error: cErr } = await supabase
            .from("clientes")
            .select("id, nome")
            .order("nome", { ascending: true });

          if (!cErr && mounted) {
            setClientes((lista ?? []) as ClienteOption[]);
          }
        }
      } catch (e: unknown) {
        if (mounted)
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error) {
        setError("Não foi possível carregar o dashboard COPSOQ.");
        setRows([]);
      } else {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setClienteNome((data as ClienteRow | null)?.nome ?? "Cliente");
      setClienteLogo((data as ClienteRow | null)?.logo_url ?? null);
    })();
  ...
  const gruposOrg = useMemo<GrupoDepartamento[]>(() => {
  ...
  if (isAdmin && !effectiveClienteId) {
    return (
      <div className="space-y-6">
        <div className="bg-white border border-slate-200 rounded-lg p-6">
          <div className="flex items-start gap-3">
            <AlertCircle className="text-slate-600 mt-0.5" size={20} />
            <div>
              <p className="font-semibold text-slate-900">Dashboard COPSOQ</p>
          ...
          <div className="mt-4">
            ...
            <select
              ...
              className="mt-1 w-full max-w-md h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm"
            >
