...
exports.alreadyProcessedEvent = alreadyProcessedEvent;
exports.applyContratoUpgrade = applyContratoUpgrade;
exports.markFailOrCancel = markFailOrCancel;
exports.markPixPending = markPixPending;
exports.activateContratoFull = activateContratoFull;
const supabase_js_1 = require("@supabase/supabase-js");
const pagarme_1 = require("./pagarme");
function isPostgrestLikeError(e) {
    return typeof e === "object" && e !== null && ("message" in e || "code" in e);
}
async function insertContratoEvento(supabase, payload) {
    const baseRow = {
    ...
    const msg = isPostgrestLikeError(first.error)
        ? (first.error.message ?? "")
        : "";
    const looksLikeMissingColumn = msg.toLowerCase().includes("gateway_event_id") ||
    ...
    if (looksLikeMissingColumn) {
        ...
        if (code2 === "23505")
            return;
        if (second.error) {
            console.error("[insertContratoEvento] fallback insert error:", second.error);
        }
        return;
    }
...
function supabaseAdmin() {
    return (0, supabase_js_1.createClient)(process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { persistSession: false } });
}
async function getContrato(supabase, contratoId) {
...
async function recordWebhookEvent(supabase, params) {
...
async function alreadyProcessedEvent(supabase, contratoId, eventId) {
    if (!eventId)
        return false;
    const byCol = await supabase
        ...
        .limit(1);
    if (!byCol.error && (byCol.data?.length ?? 0) > 0)
        return true;
...
async function applyContratoUpgrade(params) {
    ...
    const { data: upgrade, error: upgradeErr } = await supabase
        ...
        .select("id, contrato_id, quantidade_adicional, limite_anterior, limite_novo, pagarme_payment_status, paid_at")
...
async function markFailOrCancel(params) {
    const { supabase, contratoId, pagarmePaymentStatus, kind } = params;
    await supabase
        .from("contratos")
        .update({
        pagarme_payment_status: pagarmePaymentStatus ?? kind,
        atualizado_em: (0, pagarme_1.nowISO)(),
    })
        .eq("id", contratoId);
}
async function markPixPending(params) {
    console.log("PIX pendente", params);
}
async function activateContratoFull(params) {
    ...
    if (params.userId && params.userId !== contrato.criado_por) {
        ...
        const { data: userRow } = await supabase
            ...
            .maybeSingle();
        if (!userRow) {
            console.warn("[activateContratoFull] userId fornecido no webhook não pertence ao contrato, ignorando:", params.userId);
    ...
    if (targetUserId) {
        const { error: userUpdateErr } = await supabase
            ...
            .update({
            ativo: true,
            updated_at: (0, pagarme_1.nowISO)(),
        })
            .eq("id", targetUserId);
        if (userUpdateErr) {
            console.error("[activateContratoFull] erro ao ativar usuário:", userUpdateErr);
